...
RSpec.describe Profile::Translator, :type => :model do
  ...
  describe '#amount_of_orders' do
    let(:start_date) {Date.yesterday}
    let(:end_date) {Date.tomorrow}

    let!(:free_order) {create :order_verbal}
    let!(:order_in_period) {create :order_verbal, assignee: translator}
    let!(:order_out_of_period) {create :order_verbal, created_at: start_date - 1.day, assignee: translator}

    let(:translator) {create :profile_translator}

    subject{translator.amount_of_orders(start_date, end_date)}

    it{is_expected.to eq 1}
  end

  describe '#walet_amount_on_the_last_date_of_query' do
    let(:start_date) {Date.yesterday}
    let(:end_date) {Date.tomorrow}

    let!(:free_transaction) {create :transaction}
    let!(:credit_transaction) {create :transaction, credit: translator.user, sum: 15}
    let!(:debit_transaction) {create :transaction, debit: translator.user, sum: 25}
    let!(:debit_transaction_out_of_period) {create :transaction, debit: translator.user, sum: 25,
                                                   created_at: start_date - 1.day}
    let(:translator) {create :profile_translator}

    subject{translator.walet_amount_on_the_last_date_of_query(start_date, end_date)}

    it{is_expected.to eq -10}
  end

  describe '#num_of_client_alias' do
    let(:start_date) {Date.yesterday}
    let(:end_date) {Date.tomorrow}

    let!(:order_b) {create :order_verbal, banner: banner}
    let!(:order_r) {create :order_verbal, referral_link: referral_link}
    let!(:free_order_b) {create :order_verbal, banner: free_banner}
    let!(:out_of_period_order_b) {create :order_verbal, banner: banner,
                                         created_at: start_date - 1.day}

    let(:banner) {create :banner, user: translator.user}
    let(:referral_link) {create :referral_link, user: translator.user}
    let(:free_banner) {create :banner}


    let(:translator) {create :profile_translator}

    subject{translator.num_of_client_alias(start_date, end_date)}

    it{is_expected.to eq 2}
  end

  describe '#num_of_translator_alias' do
    let(:start_date) {Date.yesterday}
    let(:end_date) {Date.tomorrow}

    let!(:vassal) {create :user, overlord: translator.user}
    let!(:out_of_period_vassal) {create :user, overlord: translator.user,
                                       created_at: start_date - 1.day}
    let!(:free_vassal) {create :user, overlord: create(:user)}


    let(:translator) {create :profile_translator}

    subject{translator.num_of_translator_alias(start_date, end_date)}

    it{is_expected.to eq 1}
  end

  describe '#commission_of_translator' do
    let(:start_date) {Date.yesterday}
    let(:end_date) {Date.tomorrow}

    let!(:free_transaction) {create :transaction, is_commission_from: create(:user)}
    let!(:credit_transaction) {create :transaction, credit: translator.user, sum: 15, is_commission_from: create(:user)}
    let!(:debit_transaction_out_of_period) {create :transaction, debit: translator.user, sum: 25,
                                                   created_at: start_date - 1.day, is_commission_from: create(:user)}

    let(:translator) {create :profile_translator}

    # before(:each) do
    #   allow_any_instance_of(Transaction).to receive(:is_commission?).and_return true
    # end

    subject{translator.commission_of_translator(start_date, end_date)}
