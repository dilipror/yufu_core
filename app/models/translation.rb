  ...
  def self.all(target_locale)
    ...
    MONGO_MODELS.each do |temp|
      ...
      que.each do |k|
        ...
        result << Translation.new(key, target_locale, 'mongo', k[1][:en], k[1][target_locale])
      end

    end
    keys.each {|k| result << Translation.new(k, target_locale, 'redis', '', '')}
    result
        ...
        if current_user.localizations.where(name: target_locale).count == 1
          if target_locale.present?
            arr_of_arrs.drop(1).each do |record|
              tmp = Translation.new(record[0], target_locale, record[3], record[1], record[2])
